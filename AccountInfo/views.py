...
class AccountDetail(View):
    def get(self, request):
        if (request.session.get('credentials') != None and request.session['credentials'].get('account_id') != None):
            ...
            authorization_url = ApiMethods.get_flow()
...
class ChannelDetail(View):
    def get(self, request, channel_id):
        ...
        channel_detail = self.get_channel_detail('UC' + channel_id[2:])
        ...
        return render(request, "AccountInfo/channelInfo.html", context={'channel': channel,
                                                                        'page_token': next_page_token,
                                                                        'videos_list': channel.videos_list.all().order_by(
                                                                            '-published_at'),
                                                                        'is_authorized': True})
    ...
    def get_channel_detail(self, channel_id):
        ...
        channel_detail = {'views_count': response['statistics']['viewCount'],
                          ...
                          'banner_url': response['brandingSettings']['image']['bannerExternalUrl'],
...
class VideosPageDetail:
    ...
    def create_videos_page(self):
        ...
        logging.info("Cur: " + self.current_page_token)
        logging.info("Next: " + self.next_page_token)
    ...
    def get_current_page_videos(self, max_results=5):
        ...
        for video in response_info['items']:
            ...
            dislikes_count = detail_info['items'][0]['statistics'].get('dislikeCount', 0)
    ...
    def update_videos_page(self):
        ...
        for video in page.videos_list.all():
            ...
            video.dislikes_count = dislikes_count = int(detail_info.get('dislikeCount', 0))
            video.views_count = int(detail_info['viewCount'])
            video.comments_count = int(detail_info['commentCount'])
            if likes_count + dislikes_count == 0:
...
def create_video(video, channel):
    ...
    likes_count = int(detail_info['items'][0]['statistics']['likeCount'])
    dislikes_count = int(detail_info['items'][0]['statistics'].get('dislikeCount', 0))
...
def update_channels_list(request):
    ...
    data = {
        'next_page_token': account.pages_list.last().next_page_token,
        'data': {}
    }
